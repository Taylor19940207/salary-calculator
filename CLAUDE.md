...
- **明細書 Excel 出力（數值＋公式連動）**：Payslip toolbar「Excel出力」鈕 → `POST /api/payslip-xlsx`（`routes/payslipXlsx.ts`，exceljs）產出**與 PDF 同版面**的 .xlsx（teal 四段模板、A4 橫式 fit-to-page），有賞与時第 2 個工作表。**金額格是真數字**（cell schema `{label, value: string|number, includeInTotal?}`），標了 `includeInTotal` 的格進 Excel 合計式：`総支給額=SUM(支給格)`、`総控除額=SUM(控除格)`、`差引=引き算`、右上差引參照底部——**客戶在 Excel 改任何一格，合計即時連動**（`fullCalcOnLoad` 開啟）。標準賞与額・勤怠不進合計。數字格式 `#,##0.##`＋條件付き書式（整數時去尾點）。SUM＝総支給的不變式成立前提：月給/時給的 income label 都是『基本給』且支給格窮盡 gross 組成（含負欠勤控除、隱藏業績給）——改支給構成時要同步檢查。值與畫面/PDF/CSV 同一套 merged 值（含端數手動調整）。動機：客戶原本手工維護醜 Excel；この数値化＋公式は用戶自己實作
- **賞与（ボーナス）計算**：單人/多人表單內「今月は賞与あり」開關（基本情報與給与共用、計算完全分離）。後端 `bonusCalculator.ts` + `POST /api/calculate-bonus`：標準賞与額 1,000 円未満切捨て、健保/介護/子育て年度累計上限 573 萬（`priorFiscalBonusTotal` 手動輸入既払累計）、厚年單次上限 150 萬、雇用保險對**賞与實額**×0.5%、所得稅走「賞与算出率表」甲欄（oracle: `bonusTaxRateTable2026.ts`，國稅廳 15-16.xls 機械抽取，8 扶養×21 帶，全率=基本率×1.021 驗證過）＋特例（**2種で式が違う**・タックスアンサーNo.2523：①前月給与なし＝(賞与−社保)÷N→月額表→×N ②10倍超＝((賞与−社保)÷N＋前月給与)の月額表税額−前月給与の税額、×N の**差額方式**。②を①の式で計算すると累進が反映されず大幅過少になる——2026-07 に発覚・修正済み、oracle: 扶養0/前月10万/賞与1,000,001→40,500円。N=6、期間>6月は12）。所得稅用整數運算 `floor(x×round(rate×1000)/100000)` 避免浮點逐円誤差。明細＝給与+賞与 1 檔 2 頁（`.payslip-break` 改頁、`BonusPayslipBody.tsx` 無勤怠欄）、賞与支給日為公司層級欄位（可在 Payslip toolbar 逐人覆寫，純顯示不進計算）。批量版集計サマリー給与/賞与分列、賞与靠従業員コード（唯一性有驗證）與行對應。CSV 匯入不支援賞与。
